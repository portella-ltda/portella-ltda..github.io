...
static class BlockquoteFormatter
{
    public static void FormatBlockquotes(FileStream stream)
    {
        ...
        doc.Save("formatted_output.html");
    }
    ...
    private static void Format(HtmlNode p, Highlight highlight)
    {
        ...
        // Cria o novo span com o nome do destaque
        var span = HtmlNode.CreateNode($"<span style='color:{highlight.Color};'>{highlight.Name}</span>");

        // Adiciona o novo span e o marcador no começo do parágrafo
        p.PrependChild(span);
...
{
    internal static string ContentFormat(FileStream fileStream)
    {
